...
class GoalManager:
    ...
    def viapoint_update_pub(self, via_points): ##
        ...
        while 1:
            if via_points[self.viapoint_cnt] is not None:  ############error:when viapoint is None############
                next_viapoint.pose.position.x = via_points[self.viapoint_cnt][0]
                next_viapoint.pose.position.y = via_points[self.viapoint_cnt][1]
    ...
    def proximity_cb(self, msg):
        ...
        if proximity_dist < self.proximity_radius:
            self.viapoint_cnt += 1
            self.viapoint_update_pub(self.goal_list[self.goal_cnt].via_points)
        else:
            pass
