...
class GoalManager:
    def __init__(self):
        ...
        # self.goal_list = [
                        ...
                        # ]
        self.goal_list=[
            Goal(x=8.0, y=0, yaw=0, via_points=[[10, 0], [9, 0]]),
            Goal(x=5.0, y=0, yaw=0, via_points=[[7, 0], [6, 0]])
        ]
        self.goal_num = len(self.goal_list)
        self.marker_to_goal_dict ={1:0, 2:1, 8:2} # Goal: goal_idx
        self.rate = rospy.Rate(10)
        self.proximity_radius = 0.05 ##
    ...
    def viapoint_update_pub(self, via_points): ##
        ...
        next_viapoint1 = PoseStamped()
        next_viapoint2 = PoseStamped()
        

        while viapoint_idx < len_viapoints:
            self.viapoint_cnt = viapoint_idx
            if not self.via_point_update_flag:
                next_viapoint1.pose.position.x = via_points[viapoint_idx][0]
                next_viapoint1.pose.position.y = via_points[viapoint_idx][1]
                next_viapoint2.pose.position.x = via_points[viapoint_idx + 1][0]
                next_viapoint2.pose.position.y = via_points[viapoint_idx + 1][1]
            
                via_points_msg.poses = [next_viapoint1, next_viapoint2]

                self.via_pub.publish(via_points_msg)
                rospy.loginfo("current viapoint: (%f, %f)", next_viapoint1.pose.position.x , next_viapoint1.pose.position.y)
    ...
    def proximity_cb(self, msg):
        proximity_dist = math.sqrt(math.pow((msg.feedback.base_position.pose.position.x - self.goal_list[self.goal_cnt].via_points[self.viapoint_cnt][0]), 2) 
                                    + math.pow((msg.feedback.base_position.pose.position.y - self.goal_list[self.goal_cnt].via_points[self.viapoint_cnt][1]), 2))
